    ...
    .map(
      (row, index) => `
        <tr>
          ...
          <td><span class="score-with-suffix"><span class="score-badge ${getScoreClass(row.average)}">${formatScore(row.average, 2, 2)}</span> <span class="score-suffix">/ 10</span></span></td>
          <td>${row.count}</td>
        </tr>
      `
    )
