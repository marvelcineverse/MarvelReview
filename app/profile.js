...
function buildAvatarMarkup(url, imgClass) {
...
function renderProfileAvatarDisplay(url) {
  ...
  if (!el) return;
  el.innerHTML = buildAvatarMarkup(url, "avatar profile-avatar-large");
}

function renderProfileAvatarModalPreview(url) {
  const el = document.querySelector("#profile-avatar-modal-preview");
  if (!el) return;
  el.innerHTML = buildAvatarMarkup(url, "avatar media-avatar");
}
